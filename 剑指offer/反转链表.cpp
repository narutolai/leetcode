...
//1.反转链表 直接返回newhead
//2.反转链表前n个节点 需要后继者参数
//3.反转链表的m--n个节点 需要借助2。需要后继者参数
//4.反转两个节点间的节点,左闭右开。
//5.K个一组反转链表，需要借助4. k个一组反转链表可以多看看

//2022/09/25 22:31感想是 从递归函数的入参和功能来思考递归函数,
// 当我在表述K个一组反转链表的时候,
//ListNode *reverseKGroup(ListNode *head, int k) 的意思是k个一组反转以head为起点反转后面k个节点。
// 这样reverseKGroup(temp, k) 的意思就是以temp为起点返回后面K个节点就行了,你只要走出第一步