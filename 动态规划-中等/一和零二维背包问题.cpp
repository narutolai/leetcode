...
public: //二维0-1背包问题
    ...
    {
        //是max就全部初始化为0 如果是求最小值 就初始化为INT_MAX
...
//多次刷新这张dp表 刷新 刷新 刷新 刷新 刷新
//遍历次数
//总之就是画出dp表 然后把这张dp表的 每一个位置的值
//遇到动态规划,先口述一遍, 假设当前i个1 j个0 可以凑成的最大字符串数量为dp[i][j]，可以组成最多字符串的个数
//因为每一个字符串有着固定数量的0和1 ,但是 为什么是从，每换一个字符串，就刷新整个二维数组 you can imagine
//每换一个字符串就遍历整个二维数组从最大到最小